...
//		return session('user_auth_sign') == data_auth_sign($user) ? (isset($user['uid']) ? $user['uid'] : $user['id']) : 0;
		return (isset($user['uid']) ? $user['uid'] : $user['id']);
	}
}
...
function addUserLog($title, $uid, $front = false) {
	if (empty($uid)) {
		return false;
	} else {
...
function get_regname($uid = 0) {
	return M("customer")->cache(true)->where("id=%d", $uid)->getField("realname");
}
...
function get_address($uid) {
	...
	$addres_id = $login['address_id'];
//	$row       = M('address')->where("status='1' and uid='$uid'")->find();
	$row = M("address")->find($addres_id);
	return $row['province'] . $row['city'] . $row['area'] . $row['province'] . $row['address'];
}

function get_address_info($id) {
	$row = M("address")->find($id);
	return $row['province'] . $row['city'] . $row['area'] . $row['province'] . $row['address'];
}
...
function get_score($uid) {
	return M('customer')->where("id=%d", $uid)->getField('score');
}
...
function get_name($uid) {
	if ($uid) {
		return M('customer')->where("id=%d", $uid)->getField("realname");
	}
}
 ...
 * 返回优惠券可抵用金额
 */
function get_fcoupon_fee($coupon_id, $total, $order) {
	$coupon = M('fcoupon')->find($coupon_id);
	$lowfee = $coupon['lowpayment'];;//优惠券最低消费金额
	
	$uid  = is_login();
	$user = M("customer")->find($uid);
	$rank = $user['rank'];
	
	if ($lowfee < $total && ($rank == $coupon['user_rank'] || $coupon['user_rank'] == -1)) {
		$codeid = $coupon_id;//获取优惠券主键id
		$type   = $coupon['type'];
		$fee    = $coupon['price'];//获取优惠券等值金额
		//获取会员uid
...
//	}
	return safe_replace(get_client_ip());
}
 ...
 * 可使用优惠券最低消费金额
 */
function get_fcoupon_lowpayment($coupon_id) {
	$info = M('fcoupon')->find($coupon_id);
...
function get_suite_order_status($status) {
	switch ($status) {
		case \Home\Controller\SuiteController::ORDER_STATUS_OK:
			return "已确认";
		case \Home\Controller\SuiteController::ORDER_STATUS_WAITING_PAY:
			return "待付款";
		case \Home\Controller\SuiteController::ORDER_STATUS_FINISH:
			return "已完成";
		default:
			return "未知";
	}
...
function generate_card_num() {
	$card_num = null;
	do {
		$card_num = "B" . rand(0, 9) . rand(0, 9) . rand(0, 9) . rand(0, 9) . rand(0, 9) . rand(0, 9);
	} while (M("customer")->where("card_num2 = '%s' ", $card_num)->find());
	return $card_num;
}

function get_card_num($user) {
	if (!empty($user['card_num'])) {
		return $user['card_num'];
	}
	if (!empty($user['card_num2'])) {
		return $user['card_num2'];
	}
}

function cover_privacy($str) {
	$begin = substr($str, 0, 3);
	$end   = substr($str, -4);
	
	return $begin . str_repeat('*', strlen($str) - 7) . $end;
}
 ...
 * @param $order_id
 */
function back_money($order_id) {
	$order = M("order")->find($order_id);
	$uid   = $order['uid'];
	if (!$order) {
		\Think\Log::write("错误的订单号码:" . $order_id);
		return false;
	}
	$pay = M("pay")->where(" (out_trade_no='%s' or out_trade_no='%s') and out_trade_no!='' and out_trade_no is not null ", $order['tag'], $order['orderid'])->find();
	if (empty($pay['callback']) && $pay['uid'] && $pay['status'] == 1) {
		M()->startTrans();
		$s1 = M("customer")->where("id=%d", $pay['uid'])->setInc("balance", $pay['money']);
		$s2 = M("pay")->where("id=%d", $pay['id'])->setField("status", -1);
		//退款之后,去除消费记录中的金额
		if ($order_id) {
			$log = M("consume_log")->where("uid=" . $uid . " and remark like '%" . $order['orderid'] . "%'")->find();
			if ($log) {
				$s3 = M("consume_log")->where("id=%d", $log['id'])->limit(1)->save(array (
					                                                                   'money'  => 0,
					                                                                   'remark' => $log['remark'] . "-退款"
				                                                                   ));
			}
		}
		...
		if ($s1 !== false && $s2 !== false) {
			addUserLog("用户ID:{$pay['uid']}退款{$pay['money']},已经到账", $pay['uid']);
			M()->commit();
			return true;
		} else {
			M()->rollback();
	...
	} else if (!$pay['callback'] && $pay['param']) {//微信支付
		return false;
		if (!$pay['uid']) {
			$pay['uid']  = substr($pay['out_trade_no'], 16);
			$save['uid'] = $pay['uid'];
		}
		M()->startTrans();
		$param          = json_decode($pay['param'], true);
		$s1             = wxrefund($param['transaction_id'], $param['total_fee'], $param['total_fee']);
		$save['status'] = -1;
		$save['refund'] = $s1;
		$s2             = M("pay")->where("id=%d", $pay['id'])->save($save);
		if ($s1['result_code'] !== 'FAIL' && $s2 !== false) {
			addUserLog("用户ID:{$pay['uid']}退款{$pay['money']},来自微信支付,已经提交等待银行处理,交易号" . $pay['out_trade_no'], $pay['uid']);
			M()->commit();
			return true;
		} else {
			M()->rollback();
...
function wxrefund($transaction_id, $total_fee, $refund_fee) {
...
function isSigned($uid) {
	...
	return M("Sign")->where("create_time > {$time1} and create_time < {$time2} and uid={$uid}")->find();
}

function add_score($uid, $order_no, $pay_money) {
	$score = $pay_money * C('RATIO2');
	addUserLog("会员消费了{$pay_money},增加{$score}积分,订单号{$order_no}", $uid, true);
	return M("customer")->where("id=%d", $uid)->setInc("score", $score);
}