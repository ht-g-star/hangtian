...
class AddressController extends HomeController {
	...
	public function index() {
		...
		$address = M("address");
		$list    = $address->where("uid='{$user['id']}' and status >= 0")->select();
		$this->assign('list', $list);
		$this->meta_title = $user['realname'] . '的地址管理';
		$this->display();
	}
	...
	//编辑
	public function set($id = null, $pid = 0) {
		if (!is_login()) {
			$this->error("您还没有登陆", U("User/login"));
		}
		$id=I('id');
	...
	public function change($pid) {
		...
		$map["pid"] = $pid;
		if ($data = M("area")->cache(true)->where($map)->select()) {
			$this->ajaxReturn($data);
		}
	}
	...
	public function deleteAddress() {
		...
		if ($user['address_id'] == $id) {
			...
			$address = M("address"); // 实例化address对象
			$uid     = is_login();
			if ($address->where("uid='$uid' and id='$id'")->setField("status", -1)) {
				$data['msg']    = "删除成功";
				$data['status'] = 1;
				$this->ajaxreturn($data);
			} else {
	...
	public function shezhi() {
		...
		if (IS_AJAX) {
			...
			$result = M('customer')->where("id=%d", $user['id'])->setField("address_id", $aid);
			if ($result !== false) {
				$msg = "设置成功";
				$this->ajaxreturn($msg);
			} else {
...
//地址库有默认地址，有则保存
			...
			// 返回新增标识
			$data['msg'] = 'yes';
			M("customer")->where("id=%d", $uid)->setField("address_id", $addressid);
		} else {
			$data['uid']       = $uid;
			$data['status']    = 0;
			$data['time']      = NOW_TIME;
