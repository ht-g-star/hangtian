...
class PublicController extends HomeController {
	...
	public function notify() {
		...
		if ($notify) {
			...
			Log::write("支付宝return:" . print_r($info, true));
			if ($info['status']) {
				...
				if ($payinfo['status'] == 0 && $payinfo['callback']) {
					...
					if ($check !== false) {
						$payinfo['param'] ? $payinfo['param'] : array ();
						$info ? $info : array ();
						$save = array (
							'update_time' => time(),
							'status'      => 1,
							'param'       => serialize(array_merge(unserialize($payinfo['param']), $info))
						);
						M("Pay")->where(array ('out_trade_no' => $info['out_trade_no']))->save($save);
					}
				}
