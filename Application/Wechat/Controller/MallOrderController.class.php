 ...
 * 文档模型列表和详情
 */
class MallOrderController extends HomeController {
	protected $user;
	protected $openid;
	...
	public function add() {
		...
		if (!$user['address_id'] || !M("address")->find($user['address_id'])) {
			cookie('__forward__', get_current_url());
			$this->error("请先添加收货地址!", U('User/add_address'));
		}
		/* 积分兑换*/
		$this->assign('uid', $uid);
...
//			}
		...
		if ($_POST["PayType"] == "2") {
			...
			//支付页
			$this->success("订单创建成功,请在个人中心完成支付.", U('UserMallOrder/index'));
