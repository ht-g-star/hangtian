...
class ExchangeController extends HomeController {
	...
	public function index() {
		if (IS_POST) {
			...
			$Exchange->add();
			addUserLog('申请换货，商品名称' . get_good_name($goodid) . '参数' . $parameters, is_login(), true);
	...
	public function detail() {
		...
		$Exchange = M("Exchange");
		$list     = $Exchange->where("id='$id'")->find();
		$info     = M("Exchange")->where("id='$id'")->getField("backinfo");
