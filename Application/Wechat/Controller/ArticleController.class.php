...
class ArticleController extends HomeController {
	...
	public function index() {
		...
		$category = D('Home/Category')->info($pid);
		/* 分类信息 */
		$category           = $this->category();
		$cid                = D('Home/Category')->getChildrenId($pid);
		...
		$map['status'] = 1;
		/* 数据分页*/
		$list = D("Home/Document")->getLists($map, $category['list_row'], $listsort);
		$this->assign('list', $list);// 赋值数据集
		$page = D("Home/Document")->getPage($map, $category['list_row'], $listsort);
		...
		$comments = M("goods_comments");
		$comments = $comments->where("status=1")->limit("20")->order("id desc")->select();
...
//		$this->assign('recent', $recent);
	...
	public function detail($id = 0, $p = 1) {
		...
		//购买记录
		$records_count = M("shoplist")->where("goodid='%s'", $id)->count();
		$records       = M("shoplist")->alias("o")->join(C('DB_PREFIX') . "customer c on c.id=o.uid")
		                              ->where("goodid='%s'", $id)->field("o.num,o.create_time,o.parameters,c.realname")->limit(10)->select();
		$this->assign("records_count", $records_count);
		$this->assign("records", $records);


		/*获取商品所有好评*/
		$comment     = M('comment');
		$countbetter = $comment->where("status='1' and goodid='$id' and goodscore>='3'")->count(); //计算记录数
