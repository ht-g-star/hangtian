		...
		if ($status) {
			switch ($status) {
				case '1':
					...
					break;
				case '2':
					M('order')->where("id='$id'")->save(array ('status' => '2', 'send_time' => NOW_TIME));
//根据订单id获取购物清单
...
//获取购物清单数据表产品id，字段id
						...
						$num = $val['num'] > 0 ? $val['num'] : 1;
						//销量加1 库存减1
						$sales          = M('document')->where("id='$goodid'")->setInc('sale', $num);
						$stock          = M('document')->where("id='$goodid'")->setDec('stock', $num);
						$stock2         = M('document_product')->where("id='$goodid'")->setDec('stock', $num);
						$data['status'] = 2;
						unset($data['id']);
