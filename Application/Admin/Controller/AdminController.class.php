...
class AdminController extends Controller {
	...
	protected function _initialize() {
		if (I('get.theme')) {
			session('theme', I('get.theme'));
		}
		if (session('theme')) {
			C('DEFAULT_THEME', session('theme'));
		}
		// 获取当前用户ID
	...
	final protected function returnNodes($tree = true) {
		...
		if ((int)$tree) {
			$list = M('Menu')->field('id,pid,title,url,tip,hide')->where("status>-1")->order('sort asc')->select();
	...
	protected function exportExcel($expTitle, $expCellName, $expTableData, $rows_count = 0) {
		...
		for ($i = 0; $i < $dataNum;) {
			...
			$data = $expTableData[ $data_index ];
			if ($data['goods_count'] > 1) {
				for ($j = 0; $j < $cellNum; $j++) {
					if (stripos($expCellName[ $j ][0], ".") !== false) {
						for ($m = 0; $m < $data['goods_count']; $m++) {
							$index = explode(".", $expCellName[ $j ][0]);
							$pos   = $cellName[ $j ] . ($m + $i + 3);
							$objPHPExcel->getActiveSheet(0)->setCellValue($pos, " " . $data['goods'][ $m ][ $index[1] ]);
						}
					} else {
						$objPHPExcel->getActiveSheet(0)->mergeCells($cellName[ $j ] . ($i + 3) . ":" . $cellName[ $j ] . ($i + 2 + $data['goods_count']));//合并单元格
				...
				for ($j = 0; $j < $cellNum; $j++) {
					...
					if (stripos($expCellName[ $j ][0], ".") !== false) {
						$index = explode(".", $expCellName[ $j ][0]);
						$objPHPExcel->getActiveSheet(0)->setCellValue($pos, " " . $data['goods'][0][ $index[1] ]);
					} else {
						$objPHPExcel->getActiveSheet(0)->setCellValue($pos, " " . $data[ $expCellName[ $j ][0] ]);
					}
				}
				$i++;
			}
