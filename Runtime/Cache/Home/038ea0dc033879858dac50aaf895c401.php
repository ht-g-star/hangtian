                ...
                <div class="am-g" id="MemberCenterCtrl">
                    ...
                    <div class="am-tabs am-margin-left">
                        <ul class="am-tabs-nav am-nav am-nav-tabs">
                            <li><a href='<?php echo U("Hotel/allorder");?>' class="red">所有订单</a></li>
                            <li class="am-active"><a href='<?php echo U("Hotel/needpay");?>' class="red">待支付订单</a></li>
                            <!-- <li><a href='<?php echo U("Hotel/tobeshipped");?>' class="red">待发货订单</a></li> -->
                            <!-- <li><a href='<?php echo U("Hotel/tobeconfirmed");?>' class="red">待确认订单</a></li> -->
                        </ul>
                    </div>
                    <!--<div id="shaiorder">
						<span>筛选：</span>
						<a href='<?php echo U("Center/needpay");?>' <?php if(empty($ordertype)): ?>class='selected'<?php endif; ?>>全部订单</a>
						<a href='<?php echo U("Center/needpay",array("ordertype"=>1));?>' <?php if(($ordertype) == "1"): ?>class='selected'<?php endif; ?>>普通订单</a>
						<a href='<?php echo U("Center/needpay",array("ordertype"=>2));?>' <?php if(($ordertype) == "2"): ?>class='selected'<?php endif; ?>>酒店订单</a>
						<a href='<?php echo U("Center/needpay",array("ordertype"=>3));?>' <?php if(($ordertype) == "3"): ?>class='selected'<?php endif; ?>>线路订单</a>
					</div>-->
                        <?php if(empty($list)): ?><div class="personal_info_none_goods">
                            ...
                            <form action='<?php echo U("order/del");?>' method="post" name="delform">
                                ...
                                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$po): $mod = ($i % 2 );++$i;?><div class="single" onmouseover="this.className='singlehover'"
                                         onmouseout="this.className='single'">
                                        <div class="order-detail am-text-sm">
                                            <input class="ids row-selected" type="checkbox" name="tag[]"
                                                   value="<?php echo ($po["tag"]); ?>"> 订单号：<span class="num-text am-link-muted"><?php echo ($po["orderid"]); ?></span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;下单时间：<span
                                                class="num-text am-link-muted"><?php echo (date('Y-m-d H:i:s',$po["create_time"])); ?>&nbsp;
			<?php if(($po['status']) > "2"): ?><!-- <b><a href="<?php echo U('order/del?tag='.$po['tag']);?>"><img src="/Public/Home/images/iconfont-lajixiang.png" width="20"
                                                                    height="20"></a></b> --><?php endif; ?>
                                        </div>
                                        <!-- 列表详情区域开始 -->
                                        <div id="table4">
                                            <div id="table3">
			    ...
			    <span class="am-padding-left-sm">
			    	<?php if(($vo["stype"]) != "1"): if(($vo["stype"]) == "2"): ?><a href="<?php echo U('hotel/detail?id='.$vo['sid']);?>">
				     		<img src="<?php echo ($vo["spic"]); ?>" width="70" height="70"/> 
					     	</a><?php endif; ?>
		     			<?php if(($vo["stype"]) == "3"): ?><a href="<?php echo U('line/lineDetail?xl_id='.$vo['sid']);?>">
				     		<img src="<?php echo ($vo["spic"]); ?>" width="70" height="70"/> 
					     	</a><?php endif; ?>
		     			<?php else: ?>
		     			<a href="<?php echo U('Article/detail?id='.$vo['goodid']);?>">
			     		<img src="<?php echo (get_cover(get_cover_id($vo["goodid"]),'path')); ?>" width="70" height="70"/> 
			     		</a><?php endif; ?>
			    	<?php if(($vo["stype"]) != "1"): if(($vo["stype"]) == "2"): ?><span style="width:170px;" class="am-inline-block am-text-sm"><a href="<?php echo U('hotel/detail?id='.$vo['sid']);?>"><?php echo ($vo["sname"]); ?></a></span><?php endif; ?>
			    		<?php if(($vo["stype"]) == "3"): ?><span style="width:170px;" class="am-inline-block am-text-sm"><a href="<?php echo U('line/lineDetail?xl_id='.$vo['sid']);?>"><?php echo ($vo["sname"]); ?></a></span><?php endif; ?>
				    	<?php else: ?>
				    	<span style="width:170px;" class="am-inline-block am-text-sm"><a href="<?php echo U('Article/detail?id='.$vo['goodid']);?>"><?php echo (mb_substr(get_good_name($vo["goodid"]),0,10,'utf-8')); ?></a></span><?php endif; ?>
					 <span class="dd"><?php echo ($vo["parameters"]); ?></span>
			    </span>
                                                        <span class="am-text-warning am-padding-left">￥<?php echo ($vo["price"]); ?></span>
                                                        <span class="c2"><?php echo ($vo["num"]); ?></span>
				<span class="c2">  <?php if($vo['status']==1||$vo['status']==2){echo "" ;}; if($vo['status']==3){echo "<a href='index.php?s=/Home/back/index/id/".$vo[' id']."'>退货</a>
                    ...
                    &nbsp;";}; if($vo['iscomment']==1){echo "<br/><a href='index.php?s=/Home/comment/index/id/".$vo[' id']."'>评价商品</a>";}; if($vo['iscomment']==2){echo "<br/><a href='index.php?s=/Home/comment/index/id/".$vo[' id']."'>已评价</a>";}; ?></span>
                                            ...
                                            <div class="total-area">
												...
												<?php if(($po["ordertype"]) == "1"): ?><p class="am-padding-0 am-margin-0 am-text-xs">(运费￥<?php echo ($po["ship_price"]); ?>)</p><?php endif; ?>
                                            </div>
                                            <div class="deal-area">
                                                <p class="red am-text-sm">
                                                    <?php $status=$po['status']; if($status==-1){ echo "待支付";}; if($status==1){ echo "已提交订单";}; if($status==2){ echo "已支付";}; if($status==3){ echo "交易成功";}; if($status==4){ echo "申请取消订单";}; if($status==5){ echo "取消被拒绝";}; if($status==6){ echo "订单已取消";}; ?>
                                                </p>
                                                <p class="am-text-sm"><a
                                                        href="<?php echo U('order/detail?id='.$po['orderid']);?>">订单详情</a></p>
                                            </div>
                                            <div class="act-area ">
                                                <?php $status=$po['status']; if($status==2){ echo " <p>
                                                    <!-- <a class=' confirm'
