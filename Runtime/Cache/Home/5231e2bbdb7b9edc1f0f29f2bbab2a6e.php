...
<!-- /头部 -->
        ...
        <!-- Contents  -->
<div class="ml_contentam-g am-container am-margin-top-lg">
	...
	<div class="goodlist-main am-g am-g-collapse am-padding-left am-padding-right am-cf am-margin-top-0">
	   <div class="lists-area am-g">
		   <div class="list-fl-content am-margin-bottom">
		<!--    <?php if(!empty($bdata)): ?><ul class="brand-list am-cf am-margin-bottom-0 am-text-xs">
				   <li class="bg-f5 list-left">品牌:</li>
				   <li id='brand-all'
				   <?php if(($brandid) == ""): ?>class="active"
					   <?php else: endif; ?>
				   ><a href="<?php echo U('Article/index',array(pid=>$ctg['id'],order=>1,'sort'=>$see,'start_price'=>$start_price,'end_price'=>$end_price));?>">全部</a></li>
				   <?php if(is_array($bdata)): $k = 0; $__LIST__ = $bdata;if( count($__LIST__)==0 ) : echo "暂时没有数据" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><li id='brand-item'
					   <?php if(($brandid) == $vo['id']): ?>class="active"
						   <?php else: endif; ?>
					   ><a href="<?php echo U('Article/index',array(pid=>$ctg['id'],'brandid'=>$vo['id'],order=>1,'sort'=>$see,'start_price'=>$start_price,'end_price'=>$end_price));?>"
						   class="<?php echo ($value); ?>"><?php echo ($vo["title"]); ?></a></li><?php endforeach; endif; else: echo "暂时没有数据" ;endif; ?>
			   </ul><?php endif; ?> -->
			   <ul class="brand-list am-cf am-text-xs">
				   <li class="bg-f5 list-left">价格:</li>
				   <li
				   <?php if(($start_price) == ""): ?>class="active"
					   <?php else: endif; ?>
				   ><a href="<?php echo U('Article/index',array(pid=>$ctg['id'],'brandid'=>$brandid,order=>$order,'sort'=>$see));?>"
					   class="<?php echo ($value); ?>">全部</a></li>

				   <li
				   <?php if(($start_price) == "0"): ?>class="active"
					   <?php else: endif; ?>
				   ><a href="<?php echo U('Article/index',array(pid=>$ctg['id'],'brandid'=>$brandid,order=>$order,'sort'=>$see,start_price=>0,'end_price'=>100));?>"
					   class="<?php echo ($value); ?>">0-100</a></li>
				   <li
				   <?php if(($start_price) == "100"): ?>class="active"
					   <?php else: endif; ?>
				   ><a href="<?php echo U('Article/index',array(pid=>$ctg['id'],'brandid'=>$brandid,order=>$order,'sort'=>$see,start_price=>100,'end_price'=>200));?>"">100-200</a></li>
				   <li
				   <?php if(($start_price) == "200"): ?>class="active"
					   <?php else: endif; ?>
				   ><a href="<?php echo U('Article/index',array(pid=>$ctg['id'],'brandid'=>$brandid,order=>$order,'sort'=>$see,start_price=>200,'end_price'=>300));?>"">200-300</a></li>
				   <li
				   <?php if(($start_price) == "300"): ?>class="active"
					   <?php else: endif; ?>
				   ><a href="<?php echo U('Article/index',array(pid=>$ctg['id'],'brandid'=>$brandid,order=>$order,'sort'=>$see,start_price=>300,'end_price'=>400));?>"
					   class="<?php echo ($value); ?>">300-400</a></li>
				   <li
				   <?php if(($start_price) == "400"): ?>class="active"
					   <?php else: endif; ?>
				   ><a href="<?php echo U('Article/index',array(pid=>$ctg['id'],'brandid'=>$brandid,order=>1,'sort'=>$see,start_price=>400,'end_price'=>500));?>"
					   class="<?php echo ($value); ?>">400-500</a></li>
				   <li
				   <?php if(($start_price) == "500"): ?>class="active"
					   <?php else: endif; ?>
				   ><a href="<?php echo U('Article/index',array(pid=>$ctg['id'],'brandid'=>$brandid,order=>$order,'sort'=>$see,start_price=>500));?>"
					   class="<?php echo ($value); ?>">500以上</a></li>
			   </ul>
		   </div>
		   <div class="am-margin-bottom">
			   <ul class="ml_content_top am-cf">
				   <li
				   <?php if(($order) == "1"): ?>class="active"
					   <?php else: endif; ?>
				   ><a rel="nofollow" href="<?php echo U('Article/index',array(pid=>$ctg['id'],'brandid'=>$brandid,order=>1,'sort'=>$see,'start_price'=>$start_price,'end_price'=>$end_price));?>
		   " class="<?php echo ($value); ?>">热度<i></i></a></li>
				   <li
				   <?php if(($order) == "2"): ?>class="active"
					   <?php else: endif; ?>
				   ><a href="<?php echo U('Article/index',array(pid=>$ctg['id'],'brandid'=>$brandid,order=>2,'sort'=>$see,'start_price'=>$start_price,'end_price'=>$end_price));?>
		   " class="<?php echo ($value); ?>">最新<i></i></a></li>
				   <li
				   <?php if(($order) == "3"): ?>class="active"
					   <?php else: endif; ?>
				   ><a href="<?php echo U('Article/index',array(pid=>$ctg['id'],'brandid'=>$brandid,order=>3,'sort'=>$see,'start_price'=>$start_price,'end_price'=>$end_price));?>
		   " class="<?php echo ($value); ?>">价格<i></i></a></li>
				   <li
				   <?php if(($order) == "4"): ?>class="active"
					   <?php else: endif; ?>
				   ><a href="<?php echo U('Article/index',array(pid=>$ctg['id'],'brandid'=>$brandid,order=>4,'sort'=>$see,'start_price'=>$start_price,'end_price'=>$end_price));?>
		   " class="<?php echo ($value); ?>">销量<i></i></a></li>
			   </ul>
			</div>
		   <ul class="goodlist am-cf">
			   <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "暂时没有数据" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><li class="face" onmouseover="this.className='facehover'" onmouseout="this.className='face'">
					   <a href="<?php echo U('Article/detail?id='.$list['id']);?>" class="list-img"> <img
							   src="<?php echo (get_cover($list["cover_id"],'path')); ?>"/></a>

					   <p class="am-text-center t2"><a href="<?php echo U('Article/detail?id='.$list['id']);?>"> <?php echo ($list["title"]); ?></a></p>

					   <p class="am-text-warning am-text-center num-text" title="预览：<?php echo (get_good_price($list["id"])); ?>">￥<?php echo ($list["price"]); ?></p>

				   </li><?php endforeach; endif; else: echo "暂时没有数据" ;endif; ?>
		   </ul>
		  	<!-- 分页 -->
			<div class="page am-margin am-padding-bottom-xl am-text-right">
				<?php echo ($page); ?>
			</div>
