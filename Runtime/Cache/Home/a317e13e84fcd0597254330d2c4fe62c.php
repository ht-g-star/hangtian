			...
			<div id="memberSuiteOrder" class="userCenter-r am-u-sm-9 am-padding-xl"><!-- 个人中心 初始状态 start -->
				<div class="am-g" id="MemberSuiteOrderCtrl">
					<div class="am-u-sm-12">
						<!--标题-->
<div class="am-g">
	<div class="am-u-sm-6"><h2 class="am-fl">体检预约</h2></div>
</div>
<!--标题-->
<div class="am-tabs">
	<ul class="am-tabs-nav am-nav am-nav-tabs ">
		<li class='<?php if((ACTION_NAME) == "allorder"): ?>am-active<?php endif; ?>'>
			<a href='<?php echo U("SuiteOrder/allorder");?>' class="red">所有</a>
		</li>
		<li class='<?php if((ACTION_NAME) == "needpay"): ?>am-active<?php endif; ?>'>
			<a href='<?php echo U("SuiteOrder/needpay");?>' class="red">待支付</a>
		</li>

		<!--<li class='<?php if((ACTION_NAME) == "tobeconfirmed"): ?>am-active<?php endif; ?>'>-->
			<!--<a href='<?php echo U("SuiteOrder/tobeconfirmed");?>' class="red">待确认</a>-->
		<!--</li>-->
		<li class='<?php if((ACTION_NAME) == "tobeuse"): ?>am-active<?php endif; ?>'>
			<a href='<?php echo U("SuiteOrder/tobeuse");?>' class="red">待使用</a>
		</li>
		<li class='<?php if((ACTION_NAME) == "finished"): ?>am-active<?php endif; ?>'>
			<a href='<?php echo U("SuiteOrder/finished");?>' class="red">已完成</a>
		</li>
	</ul>
</div>
					</div>
					<div class="am-u-sm-12">
						<table class="am-table">
							<?php if(empty($list)): ?><p class="am-text-center am-padding-xl">没有待支付体检预约</p>
								<?php else: ?>
								<thead>
								<tr>
								...
								<tbody>
								<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$po): $mod = ($i % 2 );++$i;?><tr class="single" onmouseover="this.className='singlehover'" onmouseout="this.className='single'">
		<td class="order-detail">

				<!--<input class="ids row-selected" type="checkbox" name="ids[]" value="<?php echo ($po["id"]); ?>">-->
			<label>
				<span class="am-text-lg am-padding-left num-text"><?php echo ($po["title"]); ?></span>

			</label><br>
			<span class="am-text-sm am-link-muted am-padding-left num-text">订单编号:<?php echo ($po["order_no"]); ?></span><br>
			<span class="am-text-sm am-link-muted am-padding-left num-text">下单时间：<?php echo (date('Y-m-d H:i:s',$po["ctime"])); ?></span>

			<!-- 列表详情区域开始 -->
			<!-- 左边商品区域开始 -->
		<td class="c2  num-text"><?php echo ($po["count"]); ?></td>
		<td class="three-area am-text-warning num-text">
			￥<?php echo ($po["cost"]); ?>
		</td>

		<td class="deal-area">
			<p class="red">
				<?php switch($po["status"]): case "0": ?>取消<?php break;?>
					<?php case "1": ?>待支付<?php break;?>
					<?php case "10": ?>待登记<?php break;?>
					<?php case "11": ?>待使用<?php break;?>
					<?php case "20": ?>完成<?php break; endswitch;?>
			</p>
		</td>
		<td class="act-area">
			<a href="<?php echo U('SuiteOrder/detail?id='.$po['id']);?>">查看详情</a>
			<?php if($po['status'] == 1): ?>|
				<a class='confirm' href="<?php echo U('Home/Pay/index',array('so_id'=>$po['id']));?>">前去支付</a><?php endif; ?>
			<?php if($po['status'] == 20): ?><!--<a class='confirm' href="<?php echo U('SuiteOrder/detail',array('id'=>$po['id']));?>">查看报告</a>-->
				<!--<a class='confirm' href="<?php echo U('SuiteOrder/comment',array('id'=>$po['id']));?>">评价</a>--><?php endif; ?>
		</td>
		<!-- 列表详情区域结束 -->
