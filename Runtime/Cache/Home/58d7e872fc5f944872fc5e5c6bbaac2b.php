	...
	<div class="am-g am-container">
		...
		<div class="am-g am-padding am-margin">
			<div class="bg-f5 am-padding-xl">
				...
				<form action="<?php echo U('Home/Suite/order_confirm');?>" method="post" class=" am-form-inline ">
					<input type="hidden" value="<?php echo ($num); ?>" name="num" id="num">
					<input type="hidden" value="<?php echo ($s_id); ?>" name="s_id" id="s_id">
					<input type="hidden" value="<?php echo ($p_id); ?>" name="p_id" id="p_id">
					<input type="hidden" value="<?php echo ($time); ?>" name="time" id="time">
					<input type="hidden" value="<?php echo ($sex); ?>" name="sex" id="sex">
					<?php $user = $_SESSION['onethink_home']['user_auth']; ?>
					<div class="am-g">
						<?php $__FOR_START_24850__=0;$__FOR_END_24850__=$num;for($i=$__FOR_START_24850__;$i < $__FOR_END_24850__;$i+=1){ ?><div class="am-u-sm-12 ">
								<input class="am-form-field am-margin-bottom-xs" type="text" required value="<?php echo ($user["realname"]); ?>" name="realname[<?php echo ($i); ?>]" placeholder="请输入真实姓名"/>
								<input class="am-form-field am-margin-bottom-xs" type="text" required value="<?php echo ($user["mobile"]); ?>"   name="tel[<?php echo ($i); ?>]" maxlength="11" placeholder="请输入联系手机号"/>
								<input class="am-form-field am-margin-bottom-xs id_num" type="text" required value="<?php echo ($user["id_num"]); ?>" maxlength="18" name="id_num[<?php echo ($i); ?>]"  placeholder="请输入身份证号码"/>

								<div class="form-select am-inline-block">
									<select disabled >
										<option value="1" <?php if(($sex) == "1"): ?>selected<?php endif; ?>>男</option>
										<option value="0" <?php if(($sex) == "0"): ?>selected<?php endif; ?>>女</option>
									</select>
								</div>
