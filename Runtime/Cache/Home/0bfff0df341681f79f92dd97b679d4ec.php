			...
			<div class="am-g am-g-fixed am-g-collapse">
				...
				<div class="am-u-lg-12">
					<div class="qu_box clear">
						...
						<input type="hidden" value="<?php if(!empty($sectionid)): echo ($sectionid); endif; ?>" id="section"/>
						<input type="hidden" value="<?php if(!empty($datetime)): echo ($datetime); endif; ?>" id="datetime"/>
						<!--row1-->
						<span class="am-u-lg-1">目的地：</span>
						<div class="am-u-lg-11">
							<div class="tab_bar clear am-avg-lg-8">
								<li data-sq="0" class="am-active active">全部<span>(<?php echo ($linecity); ?>)</span></li>
								<?php if(is_array($cache)): $i = 0; $__LIST__ = $cache;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li data-sq="<?php echo ($vo["section_id"]); ?>"><?php echo ($vo["section_name"]); ?><span>(<?php echo ($vo["section_num"]); ?>)</span></li><?php endforeach; endif; else: echo "" ;endif; ?>
							</div>
							<div class="tab_cnt clear">
								<!--
									...
									data-city 城市id
								-->
								<span style="width: 10%;" <?php if(!$cityid): ?>class='active'<?php endif; ?>>全部<span>(<?php echo ($linecity); ?>)</span>
								</span>
								<?php if(is_array($cache)): $i = 0; $__LIST__ = $cache;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo1): $mod = ($i % 2 );++$i; if(is_array($vo1["city"])): $i = 0; $__LIST__ = $vo1["city"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo2): $mod = ($i % 2 );++$i;?><span data-sq="<?php echo ($vo1["section_id"]); ?>" style="width: 10%;" data-city="<?php echo ($vo2["city_id"]); ?>" data-section='<?php echo ($vo1["section_id"]); ?>' <?php if($cityid == $vo2['city_id']): ?>class='active'<?php endif; ?>><?php echo ($vo2["city_name"]); ?><span>(<?php echo ($vo2["city_num"]); ?>)</span></span><?php endforeach; endif; else: echo "" ;endif; endforeach; endif; else: echo "" ;endif; ?>
							</div>
						</div>
						...
						<div class="am-u-lg-11 xz_cxdate">
							...
							<?php if(is_array($cfsj)): $i = 0; $__LIST__ = $cfsj;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cf): $mod = ($i % 2 );++$i;?><button class="timebtn<?php echo ($cf["cfrq"]); ?> am-btn am-btn-sm am-btn-trans datebtn" data-date='<?php echo ($cf["cfrq"]); ?>'><?php echo ($cf["cfrqstr"]); ?></button><?php endforeach; endif; else: echo "" ;endif; ?>
						</div>
						...
						<div class="am-u-lg-4 am-u-end am-input-group">
							<div class="am-input-group">
								<input type="text" class="am-form-field" id="keyword" value="<?php if(!empty($keyword)): echo ($keyword); endif; ?>">
				...
				<div class="am-u-lg-12 line_group">
					<?php if(is_array($linelist)): $i = 0; $__LIST__ = $linelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><!--row start-->
						<div class="line_row clear">
							<div class="am-u-lg-8 line_img_box">
								<img src="<?php echo ($list["xl_list_img"]); ?>">
							</div>
							<div class="am-u-lg-4 line_info_box">
								<h3 class="title18">
									<a href="<?php echo U('Home/line/lineDetail/xl_id/'.$list['xl_id']);?>"><?php echo ($list["xl_name"]); ?></a>
								</h3>
								<?php if($list['xl_fb_detail']): ?><p><img src="/Public/Home/images/line_dt_zan.png"><?php echo ($list["xl_fb_detail"]); ?></p><?php endif; ?>
								<p><img src="/Public/Home/images/line_dt_time.png">行程天数：<?php echo ($list["xl_cttsws"]); ?></p>
								<p><img src="/Public/Home/images/line_dt_rs.png">出团人数：<?php echo ($list["xl_ctrs"]); ?>人起</p>
								<p class="line_prs"><span><?php echo ($list["xl_qijia"]); ?></span><?php echo ($list["xl_qijia_dw"]); ?></p>
							</div>
						</div>
