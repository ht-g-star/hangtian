...
$("a[href='" + url + "']").addClass("am-active");
		...
		<div class="am-g" id="MemberCenterCtrl">
			...
			<div class="am-tabs am-margin-left">
				<ul class="am-tabs-nav am-nav am-nav-tabs">
					<li><a href='<?php echo U("Center/allorder");?>'>所有订单</a></li>
					<li><a href='<?php echo U("Center/needpay");?>'>待支付订单</a></li>
					<li class="am-active"><a href='<?php echo U("Center/tobeshipped");?>'>待发货订单</a></li>
					<li><a href='<?php echo U("Center/tobeconfirmed");?>'>待确认订单</a></li>
				</ul>
		    </div>


	<?php if(empty($list)): ?><p class="am-text-center am-padding-xl">没有订单，<a href='<?php echo U("shop/index");?>' class="am-text-warning">快去下单吧</a></p>
	<?php else: ?><form action='<?php echo U("order/del");?>' method="post" name="delform" >
	  ...
	  <span class="good-status">交易状态</span> <span class="deal-action">交易操作</span></div>
          ...
          <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$po): $mod = ($i % 2 );++$i;?><div class="single" onmouseover="this.className='singlehover'" onmouseout="this.className='single'">
			   <div class="order-detail am-text-sm"><input class="ids row-selected" type="checkbox" name="tag[]" value="<?php echo ($po["tag"]); ?>">订单号：<span class="am-link-muted num-text"><?php echo ($po["orderid"]); ?></span>&nbsp;&nbsp;&nbsp;下单时间：<span class="am-link-muted num-text"><?php echo (date('Y-m-d H:i:s',$po["create_time"])); ?>&nbsp;
		...
		<!-- 左边商品区域开始 -->  
		  ...
		  <?php if(is_array($po['id'])): $i = 0; $__LIST__ = $po['id'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="cos">
		     <span class="am-padding-left-sm"><A href="<?php echo U('Article/detail?id='.$vo['goodid']);?>" class="dl"> <img src="<?php echo (get_cover(get_cover_id($vo["goodid"]),'path')); ?>"  width="70" height="70"/></a>
		   <span style="width:170px;" class="am-inline-block am-text-sm"><a href="<?php echo U('Article/detail?id='.$vo['goodid']);?>"> <?php echo (get_good_name($vo["goodid"])); ?></a></span>
 ...
 <div class="total-area"><p class="am-text-warning num-text">￥<?php echo ($po["total_money"]); ?></p><p class="am-padding-0 am-margin-0 am-text-xs">(运费￥<?php echo ($po["ship_price"]); ?>)</p>
		    </div>
		     <div class="deal-area">
			...
			<p class="am-text-sm"><a href="<?php echo U('order/detail?id='.$po['orderid']);?>" >订单详情</a></p>
			  </div>
			   <div class="act-area am-padding-0 am-margin-0 am-text-sm">
			   <?php $status=$po['status']; if($status==2){ echo " <p><a class=' confirm' href='index.php?s=/Home/order/complete/id/".$po['orderid']."'>确认收货</a> <A href='index.php?s=/Home/cancel/index/id/".$po['orderid']."'>取消订单</a></p>"; }; $pay=$po['ispay']; if($pay==1&&$po['status']==-1){ echo " <p><A href='index.php?s=/Home/cancel/index/id/".$po['orderid']."'>取消订单</a>&nbsp; &nbsp;&nbsp; &nbsp; </p>"." <p><A class='confirm' href='index.php?s=/Home/Pay/index/orderid/".$po['orderid']."'>前去支付</a> </p>"; }; if($pay==1&&$po['status']==1 ){ echo " <p><A  href='index.php?s=/Home/cancel/index/id/".$po['orderid']."'>取消订单</a> </p>"; } ; if($po['status']==2){ echo " <p><A class='red' href='index.php?s=/Home/order/detail/id/".$po['orderid']."'>查看物流</a> </p>"; } ?>
