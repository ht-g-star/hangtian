...
<head lang="en">
	...
	<style>
		...
		ul#suite_wrapper li.am-active {
			background-color: #fff;
		}
		ul#suite_wrapper li:hover {
			cursor: pointer;
		}
		ul#suite_wrapper li.selected {
			background-color: #fff;
			border:1px solid #73b4f8;
		}
...
<!-- /头部 -->
	...
	<div class="am-g am-container  am-margin-top-lg">
		...
		<div class="am-g am-container">
			<ul class="am-avg-sm-5 " id="suite_wrapper">
				<?php if(is_array($suites)): $i = 0; $__LIST__ = $suites;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$s): $mod = ($i % 2 );++$i;?><li class="suite am-padding-top-lg am-padding-bottom" data-id="<?php echo ($s["id"]); ?>">
						<span></span>
						<div class="am-u-sm-12 am-text-center"><?php echo ($s["title"]); ?></div>
						<div class="am-u-sm-12 am-text-center am-text-danger">男:￥<?php echo ($s["price_sex_1"]); ?></div>
						<div class="am-u-sm-12 am-text-center am-text-danger">女:￥<?php echo ($s["price_sex_0"]); ?></div>
					</li><?php endforeach; endif; else: echo "" ;endif; ?>
			</ul>
		</div>
		<div class="am-u-sm-2 am-u-sm-centered am-margin-lg am-padding-lg">
			<button class="am-btn am-btn-primary am-padding-left-xl am-padding-right-xl" id="next">下一步</button>
		</div>
	</div>
	<!--content end-->
	<!-- /主体 -->
...
<div class="hidden am-hide"><!-- 用于加载统计代码等隐藏元素 -->
	...
	<script>
		$("ul#suite_wrapper li.suite").hover(function () {
			$(this).toggleClass("am-active");
		});
		$("ul#suite_wrapper li.suite").click(function () {
			$("ul#suite_wrapper li.suite").removeClass("selected");
			$(this).addClass("selected");
		});
		$("#next").click(function () {
			var id = $("ul#suite_wrapper li.selected").data("id");
			if(!id){
			    topAlert("您必须选择一个套餐!");return false;
			}
			location.href = "<?php echo U('Home/Suite/detail');?>" + "&id=" + id;
		});
	</script>
