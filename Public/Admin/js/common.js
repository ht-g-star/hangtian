...
;
$(function () {
    var w = $("div.am-btn-toolbar").parent().parent().width();
    $("div.admin-content").scroll(function () {
        var st = $("div.admin-content").scrollTop();
        var dv = $("div.am-btn-toolbar").parent().parent();

        if (st >= parseInt(50)) {
            if (dv.css('position') != 'fixed') {
                dv.css({'position': 'fixed', top: 50, "width": w, "z-index": 999, "background-color": '#fff'});
            }
...
function topAlert(msg, success, autoClose) {
    ...
    $('<div  class="am-alert top-alert ' + success + ' " data-am-alert><button type="button" class="am-close">&times;</button><p>' + msg + '</p></div>').appendTo('body').alert();
    if (autoClose !== false) {
        setTimeout(function () {
